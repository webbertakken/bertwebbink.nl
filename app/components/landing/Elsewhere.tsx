...
export function Elsewhere({ data }: { data: ElsewhereContent | null }) {
  ...
  const titleAttr = dataAttr({ id: ID, type: 'elsewhere', path: 'title' }).toString()
  const groupsAttr = dataAttr({ id: ID, type: 'elsewhere', path: 'groups' }).toString()
  ...
  return (
    <main className="max-w-[1240px] mx-auto px-6 md:px-12 pt-8 pb-32" data-screen-label="elsewhere">
      ...
      {data.groups && data.groups.length > 0 && (
        <div data-sanity={groupsAttr} className="grid gap-16">
                  ...
                  {group.links.map((item, i) => (
                    ...
                    >
                      <div className="min-w-0">
                        ...
                        {item.description && (
                          <p className="font-serif italic text-[15.5px] leading-[1.4] text-ink-soft m-0 mt-1">
                            {item.description}
                          </p>
                        )}
                      </div>
