...
- [x] `yarn add @sanity/document-internationalization`
- [x] `yarn add sanity-plugin-internationalized-array` (used by `score` only - see A2.2)
- [x] `yarn remove @sanity/assist`
- [x] Remove `assist` import + plugin registration from `sanity.config.ts`
- [x] Add `documentInternationalization({ supportedLanguages, schemaTypes })` to `plugins` in `sanity.config.ts`. `schemaTypes` lists every doc/singleton **except** `score`.
- [x] Add `internationalizedArray({ languages, fieldTypes: ['string', 'text'] })` to `plugins` for `score`'s field-level fields. `languages` derives from the same `LOCALES` constant.
- [x] Define shared `LOCALES` constant in `core/i18n/locales.ts` (single source of truth for `documentInternationalization`, `internationalizedArray`, `next-intl` routing, and the language picker; **no `locale` document type in Sanity**, per skill note below):
  ```ts
...
- [x] `journal` (document)
- [x] `organ` (document)
- [x] `journalPage` (singleton - see A2.1)
- [x] `organsPage` (singleton)
- [x] `scoresPage` (singleton)
- [x] `about` (singleton)
- [x] `elsewhere` (singleton)
- [x] `privacy` (singleton)
- [x] `settings` (singleton - only translatable fields: `title`, `description`, `wordmark`, `tagline`, `scoresNoticeBody`, `scoresEditionLine`)
...
| `era` | No - categorical filter key (display labels localised via UI strings) | `string` (enum) |
...
- [x] `settingsQuery`
- [x] `navSettingsQuery`
- [x] `footerContactQuery`
- [x] `organQuery`
- [x] `organPagesSlugs` - must enumerate slugs for *all* locales (one entry per locale)
- [x] `landingOrgansQuery`
- [x] `landingStatsQuery`
- [x] `landingCitiesQuery`
- [x] `archiveOrgansQuery`
- [x] `archiveOrgansCountQuery`
- [x] `aboutQuery`
- [x] `journalQuery`
- [x] `journalPagesSlugs`
- [x] `journalEntriesQuery`
- [x] `journalStatsQuery`
- [x] `journalPageQuery`
- [x] `organsPageQuery`
- [x] `scoresPageQuery`
- [x] `llmsTxtIndexQuery` - takes `$locale`; per-locale `/llms.{locale}.txt` deferred
- [x] `elsewhereQuery`
- [x] `privacyQuery`
- [x] `scoresQuery` - uses `coalesce(field[language == $locale][0].value, field[language == "nl"][0].value)` for `forInstrument`, `edition`, `blurb`; `era` is the enum filter key (kept non-localised)
- [x] `sitemapData` - emits one URL per (slug, locale) pair with `hreflang` alternates
...
- [x] `app/[locale]/(site)/page.tsx` (journal landing)
- [x] `app/[locale]/(site)/organs/page.tsx`
- [x] `app/[locale]/(site)/organs/[slug]/page.tsx`
- [x] `app/[locale]/(site)/journal/[slug]/page.tsx`
- [x] `app/[locale]/(site)/about/page.tsx`
- [x] `app/[locale]/(site)/elsewhere/page.tsx`
- [x] `app/[locale]/(site)/scores/page.tsx`
- [x] `app/[locale]/(site)/privacy/page.tsx`
- [x] `app/[locale]/(site)/layout.tsx`
- [x] Move `app/under-construction/` to stay locale-agnostic (no prefix); accessible at `/under-construction` only
...
- [x] `yarn add next-intl`
- [x] `next.config.ts` - wrap with `createNextIntlPlugin('./i18n/request.ts')`
- [x] Create `i18n/request.ts` (server config), `i18n/routing.ts` (locale list, default), `i18n/navigation.ts` (typed `Link`, `redirect`)

### B2. Compose middleware

The under-construction gate and the locale resolver must compose cleanly.

- [x] Refactor `middleware.ts` (renamed to `proxy.ts` per Next 16):
